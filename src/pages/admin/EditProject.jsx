...
const EditProject = () => {
  ...
  // coverImage: existing URL string, or { url, preview, uploading } for new upload
  ...
  const coverUploading = typeof coverImage === 'object' && coverImage?.uploading;
  const isUploading = coverUploading || newImages.some((img) => img.uploading) || newVideos.some((v) => v.uploading);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const finalCover = typeof coverImage === 'object' ? (coverImage.url || '') : (coverImage || '');
      await projectsAPI.update(id, { form, images: finalImages, videos: finalVideos, coverImage: finalCover });
      ...
      <form onSubmit={handleSubmit} className="admin-form-wrap">
        ...
        <div className="admin-form-card">
          <h3>
            ...
            صورة الواجهة
            <span style={{ color: '#666', fontSize: '0.8rem', fontWeight: 400, marginRight: '8px' }}>(اختياري)</span>
            {coverUploading && (
          ...
          {/* Existing string URL or new object */}
          {coverImage && (
            <div style={{ position: 'relative', display: 'inline-block' }}>
              <img
                src={typeof coverImage === 'object' ? coverImage.preview : coverImage}
                alt="صورة الواجهة"
                style={{ width: '100%', maxWidth: '400px', borderRadius: '12px', border: '1px solid #2a2a2a', opacity: coverUploading ? 0.4 : 1 }}
              />
              {coverUploading ? (
